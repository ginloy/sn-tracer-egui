...
fn main() {
    ...
    if let Err(e) = rdev::listen(move |event| {
        match (event.name, events.last()) {
            (None, _) => {},
            (Some(s), None) => {
                events.push((s, Instant::now()));
            }
            (Some(s), Some((_, last_t))) if last_t.elapsed().as_micros() < DELAY_MICROS && s == "\r".to_string()  => {
                debug!("{}", last_t.elapsed().as_micros());
                println!("{}", events.iter().map(|(s, _)| s).cloned().collect::<String>());
                events.clear();
            }
